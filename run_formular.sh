...
export MODEL_NAME=ft_formular_swin
export SAVE_PATH=/xdfapp/yushilin/formular/result/${MODEL_NAME}
...
#192-768
#224-784
python $(which fairseq-train) --data-type formular --input-size 224-784 --user-dir ./ --task text_recognition  --arch swin_tiny_patch4_window7  --seed 1111 --optimizer adam --lr 5e-05 --lr-scheduler inverse_sqrt --warmup-init-lr 1e-8 --warmup-updates 500 --weight-decay 0.0001 --log-format tqdm  --log-interval 10 --batch-size ${BSZ} --batch-size-valid ${valid_BSZ} --save-dir ${SAVE_PATH} --tensorboard-logdir ${LOG_DIR} --max-epoch 300 --patience 20 --ddp-backend legacy_ddp --num-workers 4 --preprocess FM --update-freq 1 --pretrained-path /xdfapp/yushilin/formular/result/ft_formular_swin/swin_tiny_patch4_window7_224.pth  --fp16 --log-file train.log --data ${DATA} 
