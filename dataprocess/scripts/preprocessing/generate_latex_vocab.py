...
def main(args):
    ...
    print(vocab_sort)
    #vocab_sort = sorted(list(vocab.values()))
    vocab_out = []
    num_unknown = 0
    for word,value in vocab_sort:
        if value > parameters.unk_threshold:
            vocab_out.append(word+" "+str(value))
        else:
            num_unknown += 1
    #vocab = ["'"+word.replace('\\','\\\\').replace('\'', '\\\'')+"'" for word in vocab_out]
    #vocab = [word+" "+str(vocab[word]) for word in vocab_out]
