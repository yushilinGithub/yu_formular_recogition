...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.expected_resolution = (math.ceil(input_resolution[0] / window_size) * window_size,
                                    math.ceil(input_resolution[1] / window_size) * window_size)
        ...
        eH,eW = self.expected_resolution
        
        if self.shift_size > 0:
            # calculate attention mask for SW-MSA
 
            img_mask = torch.zeros((1, eH, eW, 1))  # 1 eH eW 1
            ...
            h_index = slice(H , eH)
            w_index = slice(W , eW)
            img_mask[:, h_index, w_index, :] = len(h_slices)
            cnt = 0
            for h in h_slices:
                for w in w_slices:
                    img_mask[:, h, w, :] = cnt
            ...
            img_mask = torch.zeros((1, eH, eW, 1))
            h_index = slice(H, eH)
            w_index = slice(W, eW)
            img_mask[:,h_index,w_index,:] = 1
    ...
    def forward(self, x):
        ...
        if H!=eH or W!=eW:
            ...
            assert pad_r == eW-W and pad_b == eH-H, "expected and computed is not match"
...
def swin_tiny_patch4_window7(pretrained,args):
    ...
    img_size = getattr(args,"input_size",(112,336))
    ...
    if pretrained and os.path.exists(args.encoder_pretrained_url):
        ...
        for k in checkpoint["model"].keys():
            if k in model_keys and model.state_dict()[k].shape == checkpoint["model"][k].shape:
                new_checkpoint[k] = checkpoint["model"][k]
        model.load_state_dict(new_checkpoint,strict=False)
    return model
@register_model
def swin_small_patch4_window7(pretrained,args):
    img_size = getattr(args,"input_size",(224,784))
    depths = getattr(args,"depths",[2, 2, 18])
    num_heads = getattr(args,"num_heads",[3, 6, 12])
    embed_dim = getattr(args,"swin_embed_dim",96)
    drop_path_rate = getattr(args,"drop_path_rate",0.1)
