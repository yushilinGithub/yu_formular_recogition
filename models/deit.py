...
__all__ = [
    ...
    'deit_base_distilled_patch16_384', 'deit_small_distilled_formular','deit_formular_irpe'
]
...
class DistilledVisionTransformer(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None, ape=False, mask_ratio=0.0,rpe_config=None):
...
def deit_base_distilled_patch16_custom_size(pretrained=False, img_size=384, **kwargs):
    ...
    return model
@register_model
def deit_small_distilled_formular(pretrained=False,img_size=(192,768),**kwargs):
    model = DistilledVisionTransformer(
...
@register_model
def deit_formular_irpe(pretrained=False,img_size=(192,768),**kwargs):
    ...
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), rpe_config=rpe_config,**kwargs)
    model.default_cfg = _cfg()

    if pretrained:
        # checkpoint = torch.hub.load_state_dict_from_url(
        #     #url="https://github.com/wkcn/iRPE-model-zoo/releases/download/1.0/deit_small_patch16_224_ctx_product_50_shared_k.pth",
        #     url =  "pretrain/deit_small_patch16_224_ctx_product_50_shared_k.pth",
        #     map_location="cpu", check_hash=True
        # )
        checkpoint = torch.load("pretrain/deit_small_patch16_224_ctx_product_50_shared_k.pth",map_location="cpu")
        # adapt 224 model to 384
        model_seq_len = model.state_dict()['pos_embed'].shape[1]
        ckpt_seq_len = checkpoint['model']['pos_embed'].shape[1]
        logger.warning('Deit load {:d} seq len to {:d}'.format(ckpt_seq_len, model_seq_len))
        if not model.ape:
            if model_seq_len <= ckpt_seq_len:
                checkpoint['model']['pos_embed'] = checkpoint['model']['pos_embed'][:, :model_seq_len, :]
            else:
...
@register_model
def deit_small_distilled_patch16_224(pretrained=False,img_size=(192,768), **kwargs):
    model = DistilledVisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
