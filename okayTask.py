...
try:
    from .models.unilm_models import UniLMDecoder
except:
    from models.unilm_models import UniLMDecoder

@register_model('okayOCR')
@register_model('okayFormular')
    ...
    def build_model(cls, args, task):
        ...
        if getattr(args, "decoder_pretrained", None).startswith('roberta2'):         
            ...
            if specified:
                ...
                logger.info('Load pre-trained decoder parameters from {}'.format(pretrained_model))
                roberta = torch.hub.load('pytorch/fairseq:main', pretrained_model)
                #roberta = torch.hub.load('pretrain/roberta.base/model.pt')
...
@register_model_architecture('okayFormular', 'deit_base_decoder_base')
...
@register_model_architecture('okayFormular', 'deit_base_decoder_large')
...
@register_model_architecture('okayOCR', 'okayOCR_base')
@register_model_architecture('okayFormular', 'beit_base_decoder_large')
...
@register_model_architecture('okayOCR', 'okayOCR_large')
@register_model_architecture('okayFormular', 'beit_large_decoder_large')
@register_model_architecture('okayFormular', 'DeiT_TR_LargeR_BEiT_Large')
...
@register_model_architecture('okayFormular', 'deit_base_decoder_large_custom_size')
...
@register_model_architecture('okayFormular', 'deit_small_distilled_formular')
def deit_formular(args):
    # DeiT Encoder  deit_base_distilled_patch16_custom_size
    args.deit_arch = getattr(args, "deit_arch", "deit_small_distilled_formular")
    # Transformer Decoder
    # args.encoder_embed_dim = 768
    nlrv4_compressed_tiny(args)
    base_transformer(args)
@register_model_architecture('okayFormular', 'swin_formular_irpe')
def deit_formular_irpe(args):
    # DeiT Encoder  deit_base_distilled_patch16_custom_size
    args.deit_arch = getattr(args, "deit_arch", "deit_formular_irpe")
    # Transformer Decoder
    # args.encoder_embed_dim = 768
    nlrv4_compressed_tiny(args)
    base_transformer(args)

@register_model_architecture('okayFormular', 'deit_formular_irpe')
def deit_formular_irpe(args):
    # DeiT Encoder  deit_base_distilled_patch16_custom_size
    args.deit_arch = getattr(args, "deit_arch", "deit_formular_irpe")
    # Transformer Decoder
...
@register_model_architecture('okayOCR', 'okayOCR_small_224')
...
@register_model_architecture('okayOCR', 'okayOCR_small')
@register_model_architecture('okayOCR', 'okayOCR_small_384')
...
class TrOCREncoder(FairseqEncoder):
    def __init__(self, args, dictionary):
        ...
        if 'custom_size' in args.deit_arch:
            ...
            self.deit = create_model(args.deit_arch, pretrained=True, img_size=(int(input_size[0]),int(input_size[1])))
        else:
