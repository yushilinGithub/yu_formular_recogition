...
try:
    from .data.data_load import SROIETextRecognitionDataset, SyntheticTextRecognitionDataset, FormularRecognitionDataset,SynthesizerRealFormular
...
class TextRecognitionTask(LegacyFairseqTask):
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if args.data_type in ["formular","formularSyn","formularHandWrite"]:

            target_dict = Dictionary.load(args.dict)
        ...
        return TextRecognitionTask(args, target_dict)
    ...
    def load_dataset(self, split, **kwargs):
        ...
        elif self.args.data_type =="formular":
            self.datasets[split] = FormularRecognitionDataset(self.data_dir,tfm,self.target_dict,input_size,split)
        elif self.args.data_type =="formularSyn":
            self.datasets[split] = SynthesizerRealFormular(self.data_dir,tfm,self.target_dict,input_size,split,self.args.background)
        elif self.args.data_type == "formularHandWrite":
            self.datasets[split] = SynthesizerRealFormular(self.data_dir,tfm,self.target_dict,input_size,split,data_loader="xunfeiHFMR")
        else:
    ...
    def build_generator(
        ...
        return seq_gen_cls(
            ...
            max_len_b=getattr(args, "max_len_b", 250),
